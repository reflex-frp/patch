...
patchMapWithMove :: Ord k => Map k (NodeInfo k p) -> Maybe (PatchMapWithMove k p)
...
patchMapWithMoveInsertAll :: Map k (PatchTarget p) -> PatchMapWithMove k p
...
unPatchMapWithMove :: PatchMapWithMove k p -> Map k (NodeInfo k p)
...
unsafePatchMapWithMove :: Map k (NodeInfo k p) -> PatchMapWithMove k p
...
patchMapWithMoveNewElements :: PatchMapWithMove k p -> [PatchTarget p]
patchMapWithMoveNewElements = Map.elems . patchMapWithMoveNewElementsMap

patchMapWithMoveNewElementsMap :: PatchMapWithMove k p -> Map k (PatchTarget p)
...
-- not changed.
patchThatSortsMapWith :: (Ord k, Monoid p) => (PatchTarget p -> PatchTarget p -> Ordering) -> Map k (PatchTarget p) -> PatchMapWithMove k p
...
patchThatChangesAndSortsMapWith :: forall k p. (Ord k, Ord (PatchTarget p), Monoid p) => (PatchTarget p -> PatchTarget p -> Ordering) -> Map k (PatchTarget p) -> Map k (PatchTarget p) -> PatchMapWithMove k p
...
patchThatChangesMap :: (Ord k, Ord (PatchTarget p), Monoid p) => Map k (PatchTarget p) -> Map k (PatchTarget p) -> PatchMapWithMove k p
...
instance (Ord k, Monoid p, Eq p, Patch p) => Semigroup (PatchMapWithMove k p) where
...
-- |Compose patches having the same effect as applying the patches in turn: @'applyAlways' (p <> q) == 'applyAlways' p . 'applyAlways' q@
instance (Ord k, Monoid p, Eq p, Patch p) => Monoid (PatchMapWithMove k p) where
